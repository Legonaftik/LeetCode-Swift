class Solution {
    ...
    func kthSmallest(_ matrix: [[Int]], _ k: Int) -> Int {
        var heap: Heap<Element> = .init { $0.value < $1.value }
        for row in 0..<min(matrix.count, k) {
            heap.insert(Element(row: row, column: 0, value: matrix[row][0]))
        }
        let columns = matrix[0].count
        for _ in 0..<(k-1) {
            let minElement = heap.remove()!
            if minElement.column < columns-1 {
                let row = minElement.row
                let column = minElement.column + 1
                heap.insert(Element(
                    row: row,
                    column: column,
